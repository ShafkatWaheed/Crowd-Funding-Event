...
def _load_cron_config() -> dict:
    ...
    defaults = {
        ...
        "event_reconcile_min": int(DEFAULTS["cron_event_reconcile_interval_min"]),
    }
    try:
        import asyncio
        ...
        async def _read():
            async with async_session_factory() as db:
                return {
                    ...
                    "event_reconcile_min": await s.get_int(db, "cron_event_reconcile_interval_min"),
                }
...
def _build_cron_jobs():
    ...
    reconcile_interval = max(1, cfg["event_reconcile_min"])
    reconcile_mins = set(range(0, 60, reconcile_interval))
    return [
