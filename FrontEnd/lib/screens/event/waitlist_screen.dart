...
class _WaitlistScreenState extends State<WaitlistScreen> {
  ...
  Future<void> _load() async {
    ...
    try {
      ...
      final regs = await api.getRegistrations(widget.eventId);
      final waitlisted =
          regs.where((r) => r['status'] == 'waitlist').toList();
      setState(() {
        _all = waitlisted;
        _applySearch();
        _loading = false;
  ...
  Future<void> _decide(int regId, String action) async {
    try {
      final api = context.read<ApiService>();
      ...
      child: Padding(
        ...
        child: Row(
          children: [
            ...
            // Actions
            FilledButton.tonal(
              onPressed: () => _decide(regId, 'approve'),
            ...
            FilledButton.tonal(
              onPressed: () => _decide(regId, 'reject'),
              style: FilledButton.styleFrom(
                backgroundColor:
