...
- **Reserved spots release:** New event fields `reserved_spots_release_percent`, `release_tier_spot_limits` (migration `zz10_reserved_spots_release_percent`). **schedule_reserved_spots_release()** in `event_jobs.py` enqueues a deferred ARQ job to zero reserved spots at a configurable percent of the window between selling_start and event start_time (default 100% = at start_time). Auto-transition selling_tickets→live now calls it instead of zeroing spots immediately; `release_tier_spot_limits` also releases per-tier spot caps. Platform setting **reserved_spots_release_percent_min** (admin Infrastructure) configures minimum allowed percent.
- **Auto-registration on ticket purchase:** `registration.auto_register()` silently registers or promotes the user when they attempt to purchase during selling_tickets/live; ticket/sales rejects only when no active registration in waitlist/approved-only events. Capacity check accounts for `total_reserved_spots` so reserved pledge guest spots don’t allow over-selling.
