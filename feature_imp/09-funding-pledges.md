...
- **Screen/Widget:** `EventDetailScreen` → `FundingCard` (self-contained: loads funding summary, shows progress, backers, deadline, min pledge). **Button UX:** Pledge, Donate, and Unpledge buttons use consistent 16px icon size and labels wrapped in FittedBox for readable scaling; styles aligned for accessibility. **Pledge receipt:** Header legibility — headerColor from AppIcons (donationColor/pledgeStatusColor) to match My Pledges filter chips; header background darkened 45% for contrast; text shadow (black38, blur 4–6) on icon, title, receipt number; same headerBg for Done and Copy Receipt. **My Pledges:** Per-status glow headers (orange/magenta/blue), chipActive filter chips with white labels, frosted stat chips and receipt button, per-event breakdown in group headers (pledged • donated • refunded), search-reactive stats.
