...
- **ticket_sold notification and capacity:** New notification type **ticket_sold**; organizers are notified on ticket purchase (see [34-in-app-notifications](34-in-app-notifications.md)). Event responses include **total tier capacity** (sum of tier capacities) for organizer dashboard capacity management. Refund requests screen: **search** for better UX.
- **Ticket share sheet:** From receipt or My Tickets, user can share a ticket as an image. **TicketShareSheetContent** (share_bottom_sheet) renders an off-screen **TicketExportCard** (RepaintBoundary), captures it as PNG, and shares via share_plus (XFile); fallback to in-memory XFile when path_provider is unavailable (e.g. web). Event list/featured include **viewer_ticket_count** (and viewer_pledge_amount_cents) when authenticated so EventCard can show "Tickets" / "Pledged" chips (see [06](06-event-discovery-search.md)).
- **Ticket scanner auto-download:** When **ConfigProvider.offlineTicketAutoDownloadEnabled** is true, TicketScannerScreen calls `_autoDownload()` on open: silently fetches and caches tickets when device is online and local cache for that context is empty (see [71-offline-sync-local-cache](71-offline-sync-local-cache.md)).
