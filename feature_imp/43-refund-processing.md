...
- **Screen/Widget:** My Tickets (stats: Pending/Refunded; filter "Refund Pending"; open receipt to request refund); Ticket Receipt screen (customer: "Request Refund" button); Refund Requests screen (organizer: list with **search**, approve/reject); Funding card (Refund Processing + poll refund-status).
