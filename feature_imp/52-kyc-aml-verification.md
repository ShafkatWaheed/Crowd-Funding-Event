...
- **Profile Screen -> Identity Verification section** ([`FrontEnd/lib/screens/profile/profile_screen.dart`](FrontEnd/lib/screens/profile/profile_screen.dart), widget [`FrontEnd/lib/widgets/kyc_section.dart`](FrontEnd/lib/widgets/kyc_section.dart)): Shows current KYC status (not_started, submitted, verified, rejected). Lists required document types (Government ID Front, Proof of Address) and optional types (ID Back, Selfie, Tax ID). Upload button per document type (file picker: jpg, png, pdf). Delete button for pending documents. "Submit for Verification" button (enabled when required docs uploaded). Shows rejection reasons if rejected. Section is hidden for admin users.
