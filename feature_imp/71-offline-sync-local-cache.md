...
- **SyncService:** `init()` skips connectivity subscription on **web** (`kIsWeb`) to avoid MissingPluginException; on other platforms subscribes to `Connectivity().onConnectivityChanged` with `onError: (_) {}` and try/catch. `dispose()` cancels subscription and timers. `pullEvents()` (first ~40 events): only **upserts** events whose status is in `_cacheableStatuses` (e.g. approved, selling_tickets, live, etc.); then calls **`db.deleteEventsNotInStatuses(_cacheableStatuses)`** to purge cached events that are no longer in cacheable statuses (e.g. completed, cancelled). `downloadTicketsForEvent(eventId)`, `pullMyTickets()` (only caches tickets whose event status is `selling_tickets` or `live`), `cacheScheduleForEvent(eventId)`, `cacheTransportForEvent(...)`, `pullSponsorTickets()`, `cacheSponsorDelegates(ticketId, data)`, `pullBookmarks()`. `pushOfflineScans()` sends unsynced OfflineScans via scanTicket API and marks them synced. `syncOnLaunch()` when online runs pullEvents, pullMyTickets, pullSponsorTickets, pullBookmarks, pushOfflineScans. `isOnline` uses connectivity_plus (on web, returns true if plugin throws). On connectivity restored, `pushOfflineScans()` is called so queued scans are flushed. **Ticket scanner auto-download:** When **ConfigProvider.offlineTicketAutoDownloadEnabled** is true (from public config key `offline_ticket_auto_download_enabled`), TicketScannerScreen on open calls a one-off pull so tickets are cached when online and cache is empty; see [55-public-config-endpoint](55-public-config-endpoint.md).
- **AppDatabase (Drift):** Local SQLite. Tables: CachedEvents, CachedVenues, CachedTicketTiers, OfflineTickets, OfflineScans, CachedMyTickets, CachedScheduleItems, CachedTransport, CachedSponsorTickets, CachedSponsorDelegates, CachedBookmarks, SyncMetadata. Upsert/replace/clear methods per table. **`deleteEventsNotInStatuses(Set<String> statuses)`** deletes rows from CachedEvents where status is not in the given set; used after pullEvents to remove events that have left cacheable statuses. On web, Drift uses sqlite3.wasm and drift_worker.js for async SQLite in the browser.
