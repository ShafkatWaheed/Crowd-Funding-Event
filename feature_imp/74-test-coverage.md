...
- **Frontend:** New screen tests: global_discounts_screen, organizer_sponsors_screen, sponsor_category_templates, sponsor_payment_receipt, sponsor_ticket_screen, sponsorship_categories_screen, ticket_sales_screen, ticket_scanner_screen, waitlist_screen. New widget tests: calendar_bottom_sheet, event_card, event_map_widget, kyc_section, searchable_dropdown, star_rating, tickets_bottom_sheet. Tests use pump_app and mocks for consistent isolation. **KYC widget tests** added/fixed for KycSection (title, accordion). **CI/CD:** GitHub Actions pipeline with self-hosted runner; run tests on PR, deploy on merge only; Flutter test in CI (stub `.env` before `flutter test` so pubspec asset resolution works); Backend: `alembic upgrade head` before pytest to create test DB schema; FIREBASE_CRED_JSON passed via env to avoid shell injection. **CI optimizations:** Tests run on PRs to **dev** as well as main. **Paths-filter:** Backend and frontend test jobs run only when `Backend/**` or `FrontEnd/**` files changed (dorny/paths-filter), shortening CI on single-area PRs. **Test fix:** MockEventProvider.registrationVersion returns non-null in tests to avoid analyzer/tests failures.
- **CI/test updates (redesigned UI and reorg):** Tests updated for FundingCard refactor and EventScheduleSection (vertical timeline) changes. Missing migration added and tests updated for redesigned UI. Test import paths fixed after screens moved to subdirectories (`receipts/`, `management/`, `pickers/`) and after widget folder reorganisation (`widgets/event/`); Flutter analyze clean after reorg.
- **Screen test mocks (viewer data and config):** **ticket_scanner_screen_test** — ConfigProvider mock added (screen reads `offlineTicketAutoDownloadEnabled` from ConfigProvider). **home_screen_test** — stub `viewerDataVersion` (HomeTab.didChangeDependencies reads it via Provider.of). All 660 Flutter tests pass.
